...
class CalculatorViewModel(
    private val calculateDocument: CalculateDocumentUseCase,
) : ViewModel() {
    ...
    fun onCustomDpiConfirmed() {
        ...
        if (dpi <= 0) return
        _state.update { it.copy(selectedDpis = it.selectedDpis + dpi, customDpi = "") }
    }
    ...
    fun onCalculate() {
        ...
        try {
            val results = calculateDocument(spec)
    ...
    private fun buildSpec(s: CalculatorUiState): DocumentSpecifications? {
        ...
        val bleed = if (s.bleedValue.isBlank()) {
            0.0
        } else {
    ...
    private fun resolveRatio(s: CalculatorUiState): DocumentAspectRatio? {
    ...
    private fun buildDpiList(s: CalculatorUiState): List<Int> {
